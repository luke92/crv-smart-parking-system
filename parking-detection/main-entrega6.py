...
from services.registrys import getRegistrys,postRegistrys
...
camarasResponse = getCamaras(token)
camaraSlots = camarasResponse

#Lectura de las camara_atente y su configuracion.
camerasPatentResponse = getCamarasPatent(token)
...
for dataCamaraPatente in cameraPatenteSlots:
	...
	detect_patent_async(camaraPatente, vehiculoInput, vehiculosCam1,camaraPatenteReader, dataCamaraPatente, frameCamaraPatente,token)
...
while True:
	...
	# PARKING DEFINITIONS
	parkingsResponse = getParkings(token)
	...
	update_frame_base64_to_api_async(oldframe1,camarasResponse[0],token)
	update_frame_base64_to_api_async(oldframe2,camarasResponse[1],token)
	...
	update_frame_draw_base64_to_api_async(frame1,camarasResponse[0],token)
	update_frame_draw_base64_to_api_async(frame2,camarasResponse[1],token)
	# ----------------------------------------------------------
	...
	for vhcam1 in vehiculosCam1:
		for parking in parkingSlots:
			if vhcam1.collide(parking.minx, parking.miny, radioParking) and parking.state==False and parking.cameraId == 1 :
				...
				registrys.append(Registry(0,str(datetime.now()),idParking,patente,True))
				postRegistrys(registrys,token)
				parking.patent = vhcam1.patente
				response = putParkings(parkingSlots,token)
	...
	for parking in parkingSlots:
		if(parking.state==True):
			...
			if changeState:
				...
				registrys.append(Registry(0,str(datetime.now()),idParking,patente,False))
				postRegistrys(registrys,token)
				parking.patent = "none"
				response = putParkings(parkingSlots,token)
