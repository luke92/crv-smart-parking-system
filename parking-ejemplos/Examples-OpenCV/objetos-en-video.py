...
# Cargamos el vídeo
if len(sys.argv) < 2:
	camara = cv2.VideoCapture("videos/parking-space-only-mov-1.mp4")
	#camara = cv2.VideoCapture("videos/parking-space-only-mov-2.mp4")
	#camara = cv2.VideoCapture("videos/parking-space-much-mov.mp4")
	#camara = cv2.VideoCapture("videos/parking_video.mp4")
else:
	camara = cv2.VideoCapture(sys.argv[1])
...
while True:
	...
	for c in contornos:
		...
		print("EjeX: " + str(x) + " EjeY: " + str(y) +" Ancho: "+ str(w) +" Alto: "+ str(h))
